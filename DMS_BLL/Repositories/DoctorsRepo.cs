...
namespace DMS_BLL.Repositories
{
    public class DoctorsRepo
    {
                ...
                if (!usersRepo.IsUpdatePhoneNumberExist(model.UserPhoneNumber, model.UserUpdatePhoneNumber))
                {
                    if (!usersRepo.IsUpdateEmailExist(model.UserEmail, model.UserUpdateEmail))
                    {
                        ...
                        if (addressID > 0)
                        {
                            ...
                            DoctorObj.D_Gender = model.Gender;
                            ...
                            DoctorObj.D_IsProfileCompleted = model.D_IsProfileCompleted;
                            DoctorObj.D_YearsOfExperience = model.DoctorYearsOfExperience;
        ...
        public tblDoctor GetDoctorByID(int modelId)
        {
            if (modelId > 0)
            {
                var reas = dbObj.GetDoctorByID(modelId);
                return reas;
            }
            else
                return null;
